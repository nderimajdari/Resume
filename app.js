...
function paginateResume(root, options = {}) {
  ...
  const candidates = Array.from(root.querySelectorAll(selector))
    .filter(el => !el.closest('.r-sidebar, .mr-sidebar, .pf-side, .cs-side'))
  ...
  candidates.forEach(el => {
    ...
    if (top > pageStart + bottomGuard && bottom > pageEnd - bottomGuard) {
      ...
      spacer.style.setProperty('--resume-page-gap', `${pageGap}px`);
      spacer.style.setProperty('--resume-page-remainder', `${Math.ceil(remainingPageSpace)}px`);
      spacer.style.setProperty('--resume-next-page-padding', `${Math.ceil(nextPageTopPadding)}px`);
        ...
        <div class="ep-info">
          ...
          <div class="ep-contacts">
            <div style="display:flex; flex-wrap:wrap; gap:10px;">
              ...
              ${p.linkedin ? `<span class="ep-c-item"><strong>LinkedIn:</strong> ${linkify('linkedin', p.linkedin)}</span>` : ''}
              ${p.website ? `<span class="ep-c-item"><strong>Website:</strong> ${linkify('website', p.website)}</span>` : ''}
            </div>
            ...
            ${[p.dob, p.pob, p.driverLicense, p.gender, p.nationality, p.civilStatus, p.customField].filter(Boolean).length ? `
              <div class="ep-c-item" style="margin-top:5px;">
                ${[
